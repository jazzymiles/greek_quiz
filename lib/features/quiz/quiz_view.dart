...
class QuizView extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    // ЕДИНЫЙ пустой стейт: если нет активных слов — показываем локализованный текст
    final dict = ref.watch(dictionaryServiceProvider);
    if (dict.activeWords.isEmpty) {
    ...
    return quizState.currentWord.when(
      ...
      error: (error, stack) {
        // Если вдруг что-то ещё упало — показываем текст ошибки,
        // но в обычной ситуации сюда не попадём из-за проверки выше.
        return Center(child: Text(error.toString(), textAlign: TextAlign.center));
      },
      data: (currentWord) {
        final correctAnswer =
        _getWordField(currentWord, settings.answerLanguage);
        final studyExample =
        ...
        isAnswerChecked ? l10n.next_button : l10n.check_button;
        final VoidCallback? onPressedAction =
        isAnswerChecked ? notifier.generateNewQuestion
            : (isAnswerSelected ? notifier.checkAnswer : null);
        // ---------------------------------
      ...
      onPressed: state.showFeedback
        ...
        if (settings.playAnswerSound) {
          ref
              ...
              .speak(option, settings.answerLanguage);
        }
